...
// ============================================================================
// 📚 [파일 개요] FocusScoreService - 6개 지표 가중 합산 집중도 점수 서비스
// ============================================================================
//
// 📌 6개 지표 가중 점수 시스템
//    단일 수치가 아닌 6개 독립 지표를 조합하여 정교한 집중도를 산출합니다.
//    ┌────────────────────┬────────┬──────────────────────────────┐
//    │ 지표                │ 가중치  │ 설명                          │
//    ├────────────────────┼────────┼──────────────────────────────┤
//    │ 기본 점수 (Base)     │  30%   │ 하이브리드 졸음 점수의 반전값     │
//    │ 깜빡임 (Blink)      │  20%   │ 분당 깜빡임 횟수 (정상: 15~20회) │
//    │ 머리 안정성 (Head)   │  15%   │ yaw/pitch 표준편차             │
//    │ EAR 안정성           │  15%   │ EAR 변동계수 (CV)              │
//    │ 연속 집중 (Duration) │  10%   │ 끊기지 않은 집중 지속 시간        │
//    │ 이벤트 감점 (Event)  │  10%   │ 최근 5분간 경고/졸음/이탈 횟수    │
//    └────────────────────┴────────┴──────────────────────────────┘
//
// 📌 롤링 윈도우 (Rolling Window) 개념
//    시계열 데이터에서 "최근 N초" 범위만 유지하는 기법입니다.
//    예) 깜빡임은 60초, 머리 안정성은 30초, 이벤트는 300초(5분) 윈도우를 사용합니다.
//    윈도우 밖의 오래된 데이터는 자동 제거되어 "현재 상태"를 반영합니다.
//
// 📌 스무딩 (이동 평균, Moving Average)
//    최종 점수에 이동 평균(크기 10)을 적용하여 프레임 간 급격한 변화를 완화합니다.
//    scoreHistory 배열에 최근 10개 점수를 유지하고 평균을 계산합니다.
//    이를 통해 순간적인 노이즈가 UI에 반영되지 않도록 합니다.
//
// 📌 표준편차 (Standard Deviation) 활용
//    머리 안정성: yaw/pitch 값의 표준편차가 작을수록 안정 → 고득점
//    EAR 안정성: 변동계수(CV = 표준편차/평균)가 작을수록 안정 → 고득점
//    표준편차가 크다 = 값이 많이 흔들린다 = 집중하지 못하고 있다고 판단합니다.
//
// ============================================================================
